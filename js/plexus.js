...
scene.add(this.lines);
    ...
    for (let i = 0; i < this.count; i++) {
        ...
        if (this.positions[i3] > 6 || this.positions[i3] < -6)
            this.velocities[i3] *= -1;

        if (this.positions[i3 + 1] > 4 || this.positions[i3 + 1] < -4)
            this.velocities[i3 + 1] *= -1;

        if (this.positions[i3 + 2] > 3 || this.positions[i3 + 2] < -3)
                ...
                for (let oz = -1; oz <= 1; oz++) {
                    ...
                    for (const i of particles) {
                        ...
                        for (const j of neighbors) {
                            ...
                            if (connectionCount[j] >= maxConnections)
                                continue;
